...
class WeightedSoftDiceLoss(torch.nn.Module):
    ...
    @staticmethod
    def forward(logits, labels, weight, **_):
...
class BCEDiceFocalLoss(nn.Module):
    ...
    '''
    def __init__(self, focal_param=0.5, weights=None, weight = None, **kwargs):
        if weights is None:
            weights = [1.0,1.0,1.0]
        super(BCEDiceFocalLoss, self).__init__()
        self.bce = BCEWithLogitsViewLoss(weight=weight, size_average=True, **kwargs)
